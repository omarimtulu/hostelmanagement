				...
				<form action="application_form.php?id=<?php echo $_GET['id']?>" method="POST">
					<div class="row">
						<div class="col-md-6 contact_left_grid" data-aos="fade-right">
							...
							<div class="contact-fields-w3ls">
							<label for="beds">available rooms</label>
							<select name="Room_No">
							<optgroup label="<?php echo  'Rooms in '; echo $_GET['id']?>">
								<?php 
									$hostel = $_GET['id'];
									if(($hostel)=='A'){ $hostel_id=1;
									}elseif(($hostel)=='B'){ $hostel_id=2;
									}elseif(($hostel)=='C'){ $hostel_id=3;
									}elseif(($hostel)=='D'){ $hostel_id=4;
									}elseif(($hostel)=='E'){ $hostel_id=5;
									}elseif(($hostel)=='F'){ $hostel_id=6;
									}
									$query1 = "SELECT * FROM Room where Hostel_id = '$hostel_id' and Allocated = '0'";
									$result1 = mysqli_query($conn,$query1);

									while($row1 = mysqli_fetch_assoc($result1)){
											 echo "<option value='.$hoste.'>".$row1['Room_No']." </option>";
										 }
								?>
							<input type="number" name="room_no" placeholder="please select an available room" required="">
							</select>
							<!--------------------places all options in an array,loop checks your options variable if it is equal it is set--------------------------------------
							<label for="beds">select room2</label>-->
							<?php
						/*	$selection=$row1['Room_No']
							echo '<select><option value="0">Select option</option>';
...
{
	$row1=$_GET['room'];
	$quer = "UPDATE Application SET Room_No = '$row1' WHERE Student_id = '$student_id'";
...
<footer class="py-5">
	<div class="container py-md-5">
		<div class="footer-logo mb-5 text-center">
			<a class="navbar-brand" href="http://hms.ac.in" target="_blank">HMS<span class="display">WELCOME</span></a>
		</div>
...
/*if(isset($_POST['submit'])){
	if (!empty($_POST['room_no'])) {
		$selected = $_POST['room_no'];
		echo "<script type='text/javascript'>alert('You have choosen $selected !!')</script>";
	}else{
		echo "<script type='text/javascript'>alert('please select a value !!')</script>";
	}
}*/
?>
...
/*    	$query_imp = "SELECT * FROM Payment WHERE Student_id = '$roll' ";
		...
		$query6 = "INSERT INTO Room (Room_id,Hostel_id,Room_No,Allocated) VALUES ('','$hostel_id','$room','0')";
		$result6 = mysqli_query($conn,$query6);
        $query3 = "INSERT INTO Application (Student_id,Hostel_id,Application_status,Room_No,Message) VALUES ('$roll','$hostel_id',true,'$room','$message')";
